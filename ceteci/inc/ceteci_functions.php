<?php
    ...
    function modNameOptList($db_conx) {
        $mod_name_list = "";
        $consulta = "SELECT * FROM module_names";
        if(!$sql = $db_conx->prepare($consulta)){
    ...
    function modStatusOptList($db_conx) {
        $mod_status_list = "";
        $consulta = "SELECT * FROM tbl_module_statuses";
        if(!$sql = $db_conx->prepare($consulta)){
            echo "Lo sentimos, estamos experimentando problemas, intentalo mas tarde.";
            echo "Fallo la preparacion (" . $db_conx->errno . ") " . $db_conx->error;
            exit;
        ...
        while($row = $resultado->fetch_assoc()) {
            $mod_status_id = $row['mod_status_id'];
            $mod_status_name = $row['mod_status_nombre'];
    ...
    function studStatOptList($db_conx) {
        $stud_status_option = "";
        $sql = "SELECT * FROM tbl_estud_estatuses";
	...
	function select_mod_cat($db_conx) {
        $mod_cat_list = null;
        $consulta = "SELECT * FROM module_categories";
        if(!$sql = $db_conx->prepare($consulta)) {
            echo "Lo sentimos, estamos teniendo problemas, intentelo mas tarde.";
            echo "Fallo en la preparacion: (" .$db_conx->errno . ") " .$db_conx->error;
            exit;
        ...
        while($row = $resultado->fetch_assoc()){
            $mod_cat_id = $row['mod_cat_id'];
            $mod_cat_name = $row['mod_cat_name'];
