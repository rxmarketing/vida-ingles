<?php
	...
	if($u == $log_username && $user_ok == true){
		...
		$avatar_form  = '<form class="form" role="form" id="avatar_form" name="avatar_form"  enctype="multipart/form-data" method="post" action="http://localhost/vidaingles/ajax/ajax_photo_system.php">';
	...
	$log_user_pic = '<img class="img-circle" src="http://localhost/vidaingles/users/'.$log_username.'/'.$avatar.'" alt="'.$log_username.'" width="27" height="27" title="'.$log_username.'">';
	if($avatar == NULL){
		$log_user_pic = '<img src="http://localhost/vidaingles/i/avatardefault.jpg" width="20" height="20" alt="'.$log_username.'">';
	}
	$profile_pic = '<img class="img-circle" src="http://localhost/vidaingles/users/'.$u.'/'.$avatar.'" alt="'.$u.'" width="200" height="200">';
	if($avatar == NULL){
		$profile_pic = '<img class="img-circle" src="i/avatardefault.jpg" alt="'.$u.'">';
	}
	$aviso = "";
	$adminNav = "";
	if ($admin_ok == true) {
		$adminNav = '<a href="http://localhost/vidaingles/admin/add_lesson.php">Add lesson</a> | <a href="admin/add_exercise.php">Add exercise</a> | <a href="admin/add_course.php">Add course</a>';
		} else {
...
<html lang="es">
	<head>
		<title><?php echo $u. ' | '. $site_name ?> </title>
		<meta charset="UTF-8">
			...
			<div class="row">
				<div class="col-md-8">
					...
					<?php 
						if ($admin_ok == true) {
						echo '<h2>Mensajes</h2>';
						echo '<div id="module-messages" class="table-responsive">';
						echo '<table class="table module-mensajes table-striped">';
						echo '<th>Fecha</th><th>Por</th><th>Celular</th><th>Email</th><th>Asunto</th>';
							include_once('admin/template_admin_rightbar.php');
						echo '</table></div>';
							}
					?>
				...
				<div class="col-md-4">
					<?php 
						if ($admin_ok == true) {
							...
							include_once('templates/template_sidebar_right.php');
						}
					?>
				</div><!--Ends col-md-4 -->
			</div>
