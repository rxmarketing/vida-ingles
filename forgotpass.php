<?php
	...
	if(isset($_GET['u']) && isset($_GET['p'])){
		...
		$sql = "SELECT userid FROM usuarios WHERE username='$u' AND temp_pass='$temppasshash' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$numrows = mysqli_num_rows($query);
		if($numrows == 0){
			header("location: message.php?msg=No existe un nombre de usuario con esta contraseña temporal. No puedo continuar.");
			exit();
			} else {
