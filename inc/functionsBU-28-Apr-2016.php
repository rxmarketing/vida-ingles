<?php
	...
	if($exam_count < 1) {
		...
		while ($row = mysqli_fetch_array($qry, MYSQLI_ASSOC)) {
			...
			$exam_list .= '<li><a href="http://localhost/vidaingles/ejercicios/'.$examid.'/'.$ejerFriendlyName.'" title="'.$ejerNoTags.'">'.$examname.'</a><span class="cef"><a href="http://localhost/vidaingles/paginas/1/marco-comun-europeo" title="Common European Framework of Reference">'.$cefname.' </a></span></li>';
      ...
      <div class="row featurette">
		...
		if($exam_count > $maxexams){
			$view_all_exams_link = '<a role="menu-item" href="http://localhost/vidaingles/ejercicios">ver todos</a>';
			} else {
	...
	if($course_count < 1) {
		...
		while ($row = mysqli_fetch_array($courselist, MYSQLI_ASSOC)) {
			...
			$course_list .= '<li><a href="http://localhost/vidaingles/cursos/'.$course_id.'/'.$couseFriendlyName.'" title="'.$course_name.'"">'.$course_name.'</a><span class="cef"><a href="vi_page.php?id=1" title="Common European Framework of Reference">'.$cef_desc.' </a></span></li>';
